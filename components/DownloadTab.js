...
export default function DownloadTab({
  ...
  const hasAutoDownloaded = useRef(false); // guard against double-fire

  // Merged single effect — avoids stale closure by using initialCode directly
  useEffect(() => {
    if (!initialCode || initialCode.length !== 4) return;
    if (hasAutoDownloaded.current) return; // already consumed this code

    hasAutoDownloaded.current = true;
    setDownloadCode(initialCode);
    handleDownload(initialCode); // pass directly, don't rely on state
    onConsumed(); // immediately clear urlCode in parent
  }, [initialCode]);

  // Accept optional codeOverride to avoid stale downloadCode state
  const handleDownload = async (codeOverride = null) => {
    const code = codeOverride ?? downloadCode;
    onError("");
    ...
    try {
      ...
      if (contentType && contentType.includes("application/json")) {
        ...
        if (!a.download) a.download = `wisp-${code}`;
  ...
  return (
    // ... rest of JSX unchanged, button calls handleDownload() with no args
